...
dampingLim=p.Results.dampingLim;
...
%% analysis of frequency content over time
for i=1:size(acc,2)
    for timePoints=1:size(intCoefficients{i},2)
        freqAnalysisTMP=frequencyEstimation(intFreq,intCoefficients{i}(:,timePoints));
        cwtParam.map.sep.time{i}.main(timePoints,:)=freqAnalysisTMP.sep.main;
        cwtParam.map.sep.time{i}.median(timePoints,:)=freqAnalysisTMP.sep.median;
        cwtParam.map.sep.time{i}.mean(timePoints,:)=freqAnalysisTMP.sep.mean;
    end
end

if size(acc,2)>1
    for timePoints=1:size(intNormCoefficients,2)
        freqAnalysisTMP=frequencyEstimation(intFreq,intNormCoefficients(:,timePoints));
        cwtParam.map.norm.time.main(timePoints,:)=freqAnalysisTMP.sep.main;
        cwtParam.map.norm.time.median(timePoints,:)=freqAnalysisTMP.sep.median;
        cwtParam.map.norm.time.mean(timePoints,:)=freqAnalysisTMP.sep.mean;
    end
end
...
cwtParam.damp=dampingEstimation(power,'calculDamping',damping,'Fs',newFs,'plotfig',plotFig,'isIMF',isIMF,'delay2peak',delay2peak, 'dampingLim',dampingLim, 'titleFig','Amplitude in the time domain');
